...
#
#   plain float32     MRX_DTYPE=float32                              (the production default, tol 1e-5)
#   plain float64     MRX_DTYPE=float64                              (tol 1e-10)
#   refined float32   MRX_DTYPE=float32 MRX_RESIDUAL_DTYPE=float64   (mixed, the paper's runs, tol 1e-8)
#
...
declare -A ENVS=([mixed]="MRX_DTYPE=float32 MRX_RESIDUAL_DTYPE=float64" [float64]="MRX_DTYPE=float64"
                 [plain32]="MRX_DTYPE=float32")

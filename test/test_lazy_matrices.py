...
def test_lazy_mass_matrix_init(simple_form_0, identity_quadrature, identity_map):
    ...
    assert M.Lambda_0 == simple_form_0, "Λ0 should match input form"
    assert M.Lambda_1 == simple_form_0, "Λ1 should match input form for mass matrix"
...
def test_lazy_mass_matrix_0form(simple_form_0, identity_quadrature, identity_map):
    ...
    assert M_assembled.shape == (
        simple_form_0.n, simple_form_0.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        M_assembled, M_assembled.T), "Mass matrix should be symmetric"
    assert jnp.all(jnp.linalg.eigvals(M_assembled) >= -
                   1e-10), "Mass matrix should be positive semi-definite"
...
def test_lazy_mass_matrix_1form(simple_form_1, identity_quadrature, identity_map):
    ...
    assert M_assembled.shape == (
        simple_form_1.n, simple_form_1.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        M_assembled, M_assembled.T), "Mass matrix should be symmetric"
...
def test_lazy_mass_matrix_2form(simple_form_2, identity_quadrature, identity_map):
    ...
    assert M_assembled.shape == (
        simple_form_2.n, simple_form_2.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        M_assembled, M_assembled.T), "Mass matrix should be symmetric"
...
def test_lazy_mass_matrix_3form(simple_form_3, identity_quadrature, identity_map):
    ...
    assert M_assembled.shape == (
        simple_form_3.n, simple_form_3.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        M_assembled, M_assembled.T), "Mass matrix should be symmetric"
...
def test_lazy_mass_matrix_vector(simple_form_vector, identity_quadrature, identity_map):
    ...
    assert M_assembled.shape == (
        simple_form_vector.n, simple_form_vector.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        M_assembled, M_assembled.T), "Mass matrix should be symmetric"
...
def test_lazy_mass_matrix_array_conversion(simple_form_0, identity_quadrature, identity_map):
    ...
    assert isinstance(
        M_array, np.ndarray), "__array__ should return numpy array"
    assert M_array.shape == (
        simple_form_0.n, simple_form_0.n), "Array shape should match matrix shape"
...
def test_lazy_derivative_matrix_gradient(simple_form_0, simple_form_1, identity_quadrature, identity_map):
    ...
    D = LazyDerivativeMatrix(simple_form_0, simple_form_1,
                             identity_quadrature, identity_map)
    D_assembled = D.matrix()

    # Matrix shape is (n1, n0) because einsum returns (l, i) = (n1, n0)
    assert D_assembled.shape == (
        simple_form_1.n, simple_form_0.n), "Matrix shape should be (n1, n0)"
...
def test_lazy_derivative_matrix_curl(simple_form_1, simple_form_2, identity_quadrature, identity_map):
    ...
    D = LazyDerivativeMatrix(simple_form_1, simple_form_2,
                             identity_quadrature, identity_map)
    D_assembled = D.matrix()

    # Matrix shape is (n1, n0) because einsum returns (l, i) = (n1, n0)
    assert D_assembled.shape == (
        simple_form_2.n, simple_form_1.n), "Matrix shape should be (n1, n0)"
...
def test_lazy_derivative_matrix_div(simple_form_2, simple_form_3, identity_quadrature, identity_map):
    ...
    D = LazyDerivativeMatrix(simple_form_2, simple_form_3,
                             identity_quadrature, identity_map)
    D_assembled = D.matrix()

    # Matrix shape is (n1, n0) because einsum returns (l, i) = (n1, n0)
    assert D_assembled.shape == (
        simple_form_3.n, simple_form_2.n), "Matrix shape should be (n1, n0)"
...
def test_lazy_derivative_matrix_3form(simple_form_3, identity_quadrature, identity_map):
    ...
    D = LazyDerivativeMatrix(simple_form_3, simple_form_3,
                             identity_quadrature, identity_map)
    D_assembled = D.matrix()
    ...
    assert D_assembled.shape == (
        simple_form_3.n, simple_form_3.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        D_assembled, 0.0), "Derivative of 3-form should be zero"
...
def test_lazy_projection_matrix(simple_form_0, simple_form_1, identity_quadrature, identity_map):
    ...
    P = LazyProjectionMatrix(simple_form_0, simple_form_1,
                             identity_quadrature, identity_map)
    P_assembled = P.matrix()

    # Matrix shape is (n1, n0) because einsum returns (l, i) = (n1, n0)
    assert P_assembled.shape == (
        simple_form_1.n, simple_form_0.n), "Matrix shape should be (n1, n0)"
...
def test_lazy_double_curl_matrix(simple_form_1, identity_quadrature, identity_map):
    ...
    assert C_assembled.shape == (
        simple_form_1.n, simple_form_1.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        C_assembled, C_assembled.T), "Double curl matrix should be symmetric"
...
def test_lazy_stiffness_matrix(simple_form_0, identity_quadrature, identity_map):
    ...
    assert K_assembled.shape == (
        simple_form_0.n, simple_form_0.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        K_assembled, K_assembled.T), "Stiffness matrix should be symmetric"
    assert jnp.all(jnp.linalg.eigvals(K_assembled) >= -
                   1e-10), "Stiffness matrix should be positive semi-definite"
...
def test_lazy_double_divergence_matrix(simple_form_2, identity_quadrature, identity_map):
    ...
    D = LazyDoubleDivergenceMatrix(
        simple_form_2, identity_quadrature, identity_map)
    D_assembled = D.matrix()

    assert D_assembled.shape == (
        simple_form_2.n, simple_form_2.n), "Matrix shape should be (n, n)"
    assert jnp.allclose(
        D_assembled, D_assembled.T), "Double divergence matrix should be symmetric"
...
def test_lazy_extraction_operator_init(simple_extraction_operator_0):
    ...
    E, Lambda = simple_extraction_operator_0

    assert E.k == Lambda.k, "Form degree should match"
    assert E.Lambda == Lambda, "Reference to form should match"
    assert E.nr == Lambda.nr, "nr should match"
    assert E.nt == Lambda.nt, "nt should match"
    assert E.nz == Lambda.nz, "nz should match"
...
def test_lazy_extraction_operator_0form(simple_extraction_operator_0):
    ...
    assert E_assembled.shape == (
        E.n, Lambda.n), "Matrix shape should be (n_extracted, n_original)"
...
def test_lazy_extraction_operator_1form(simple_extraction_operator_1):
    ...
    E, Lambda = simple_extraction_operator_1
    E_assembled = E.matrix()
...
def test_lazy_extraction_operator_2form(simple_extraction_operator_2):
    ...
    E, Lambda = simple_extraction_operator_2
    E_assembled = E.matrix()
...
def test_lazy_extraction_operator_3form(simple_extraction_operator_3):
    ...
    E, Lambda = simple_extraction_operator_3
    E_assembled = E.matrix()
...
def test_lazy_extraction_operator_array_conversion(simple_extraction_operator_0):
    ...
    assert isinstance(
        E_array, np.ndarray), "__array__ should return numpy array"
...
def test_get_xi():
    ...
    assert ξ.shape == (3, 2, nt), f"ξ should have shape (3, 2, {nt})"

    # Check that ξ[0, 0, :] + ξ[1, 0, :] + ξ[2, 0, :] = 1 (partition of unity)
    sum_xi0 = ξ[0, 0, :] + ξ[1, 0, :] + ξ[2, 0, :]
    npt.assert_allclose(sum_xi0, 1.0, atol=1e-12,
                        err_msg="Partition of unity for ξ[*, 0, :] should hold")

    # Check that values are in reasonable range
    assert jnp.all(
        ξ >= -1e-10), "ξ values should be non-negative (within tolerance)"
...
def test_get_xi_different_sizes():
    ...
    for nt in [4, 8, 16, 32]:
        ξ = get_xi(nt)
        assert ξ.shape == (
            3, 2, nt), f"ξ should have shape (3, 2, {nt}) for nt={nt}"
...
def test_extraction_operator_boundary_conditions(simple_form_0, zero_bc):
    ...
    assert E_assembled.shape == (
        E.n, simple_form_0.n), "Matrix shape should be correct"
    assert E.o == (
        1 if zero_bc else 0), "Offset should match boundary condition type"
...
def test_lazy_boundary_operator_init_0form(simple_form_0):
    ...
    B = LazyBoundaryOperator(
        simple_form_0, ("dirichlet", "periodic", "periodic"))

    assert B.k == simple_form_0.k, "Form degree should match"
    assert B.Lambda == simple_form_0, "Reference to form should match"
...
def test_lazy_boundary_operator_init_1form(simple_form_1):
    ...
    B = LazyBoundaryOperator(
        simple_form_1, ("dirichlet", "periodic", "periodic"))
...
def test_lazy_boundary_operator_init_2form(simple_form_2):
    ...
    B = LazyBoundaryOperator(
        simple_form_2, ("dirichlet", "periodic", "periodic"))
...
def test_lazy_boundary_operator_init_3form(simple_form_3):
    ...
    B = LazyBoundaryOperator(
        simple_form_3, ("dirichlet", "periodic", "periodic"))
...
def test_lazy_boundary_operator_matrix_0form(boundary_operator_0form):
    ...
    B, Lambda = boundary_operator_0form
    B_assembled = B.matrix()

    assert B_assembled.shape == (
        B.n, Lambda.n), "Matrix shape should be (n_boundary, n_original)"
    # Matrix should be sparse (mostly zeros)
    assert jnp.sum(
        B_assembled > 0) <= B.n, "Matrix should have at most n non-zero entries per row"
...
def test_lazy_boundary_operator_matrix_1form(boundary_operator_1form):
    ...
    B, Lambda = boundary_operator_1form
    B_assembled = B.matrix()
...
def test_lazy_boundary_operator_matrix_2form(boundary_operator_2form):
    ...
    B, Lambda = boundary_operator_2form
    B_assembled = B.matrix()
...
def test_lazy_boundary_operator_matrix_3form(boundary_operator_3form):
    ...
    B, Lambda = boundary_operator_3form
    B_assembled = B.matrix()
    ...
    for row in range(B.n):
        ...
        assert row_sum <= 1.0 + \
            1e-10, f"Row {row} should have at most one non-zero entry"
...
def test_lazy_boundary_operator_array_conversion(boundary_operator_0form):
    ...
    assert isinstance(
        B_array, np.ndarray), "__array__ should return numpy array"
...
def test_lazy_boundary_operator_unravel_index(boundary_operator_0form):
    ...
    for i in range(min(10, B.n)):
        ...
        assert 0 <= j_idx < B.nt, "j index should be in valid range"
        assert 0 <= k_idx < B.nz, "k index should be in valid range"
...
def test_lazy_boundary_operator_boundary_types_0form(simple_form_0, bc_type):
    ...
    assert B_assembled.shape == (
        B.n, simple_form_0.n), "Matrix shape should be correct"
...
def test_lazy_boundary_operator_dirichlet_reduction(simple_form_0):
    ...
    B_dirichlet = LazyBoundaryOperator(
        simple_form_0, ("dirichlet", "dirichlet", "dirichlet"))
    B_none = LazyBoundaryOperator(
        simple_form_0, ("periodic", "periodic", "periodic"))

    assert B_dirichlet.nr == simple_form_0.nr - 2, "Dirichlet should reduce nr by 2"
    assert B_dirichlet.nt == simple_form_0.nt - 2, "Dirichlet should reduce nt by 2"
    assert B_dirichlet.nz == simple_form_0.nz - 2, "Dirichlet should reduce nz by 2"
...
def test_lazy_boundary_operator_left_right_reduction(simple_form_0):
    ...
    B_left = LazyBoundaryOperator(
        simple_form_0, ("left", "periodic", "periodic"))
    B_right = LazyBoundaryOperator(
        simple_form_0, ("right", "periodic", "periodic"))
    B_none = LazyBoundaryOperator(
        simple_form_0, ("periodic", "periodic", "periodic"))
...
def test_lazy_boundary_operator_vector_field(simple_form_vector):
    ...
    B = LazyBoundaryOperator(
        simple_form_vector, ("dirichlet", "periodic", "periodic"))
    B_assembled = B.matrix()
    ...
    assert B_assembled.shape == (
        B.n, simple_form_vector.n), "Matrix shape should be correct"
...
def test_lazy_boundary_operator_element_computation(boundary_operator_0form):
    ...
    for row in range(min(5, B.n)):
        for col in range(min(5, Lambda.n)):
            element = B._element(row, col)
            element_val = float(element) if isinstance(
                element, jnp.ndarray) else element
            assert element_val in [
                0, 1], "Element should be 0 or 1 (binary operator)"
            assert isinstance(element, (int, jnp.ndarray)

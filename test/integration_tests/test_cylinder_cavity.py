...
def test_cylinder_cavity():
    ...
    matched_evs = []
    matched_rel_errors = []
    # Empty set to start, will track which ones have already been matched.
    used_indices = set()
    ...
    for true_ev in true_evs:
        ...
        for i, ev in enumerate(evs):
            if i not in used_indices:
                rel_error = float(jnp.abs(ev - true_ev) /
                                  true_ev)  # Relative error
    ...
    if len(matched_evs) == 0:
        raise AssertionError(
            "No computed eigenvalues matched theoretical eigenvalues.")

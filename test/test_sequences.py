...
def test_projection_0form(p):
    ...
    # where (f, Lambda_0) is computed using integrate_against with the correct weights
...
def test_differential_forms_setup(p):
    ...
    assert hasattr(Seq, 'Lambda_0'), "Λ1 not created"
    assert hasattr(Seq, 'Lambda_1'), "Λ1 not created"
    assert hasattr(Seq, 'Lambda_2'), "Λ2 not created"
    ...
    for k, Λ in enumerate([Seq.Lambda_0, Seq.Lambda_1, Seq.Lambda_2, Seq.Lambda_3]):
        ...
        assert Λ.nt == 4, f"Λ{k} has wrong nt: expected 4, got {Λ.nt}"
        assert Λ.nz == 4, f"Λ{k} has wrong nz: expected 4, got {Λ.nz}"
        assert Λ.pr == p, f"Λ{k} has wrong pr: expected {p}, got {Λ.pr}"
        assert Λ.pt == p, f"Λ{k} has wrong pχ: expected {p}, got {Λ.nt}"
        assert Λ.pz == p, f"Λ{k} has wrong pζ: expected {p}, got {Λ.pz}"
    ...
    assert val_indexed.shape == (
        1,), "Indexed basis function should return shape (1,)"
...
def test_discrete_function(p):
    ...
    val_0 = f_0(x_test)
    assert val_0.shape == (
        1,), f"DiscreteFunction(0-form) should return shape (1,), got {val_0.shape}"
    assert jnp.all(jnp.isfinite(val_0)
    ...
    assert jnp.all(jnp.isfinite(val_1)
    ...
    assert jnp.all(jnp.isfinite(val_2)
    ...
    assert jnp.all(jnp.isfinite(val_3)

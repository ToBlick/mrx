...
def test_mrx_diagnostics_pressure_non_force_free(
    ...
    # Compute grad(p) in physical space using strong_grad operator
    grad_p = seq.strong_grad @ p_hat
    assert jnp.all(jnp.isfinite(grad_p)), "Gradient of pressure should be finite"
...
def test_time_stepper_update_hessian(
    ...
    npt.assert_allclose(updated_state.Hessian, new_Hessian, rtol=1e-10, atol=1e-10)
    # Other fields should remain unchanged
...
def test_time_stepper_update_B_guess(
    ...
    npt.assert_allclose(updated_state.B_guess, new_B_guess, rtol=1e-10, atol=1e-10)
    # Other fields should remain unchanged
    assert updated_state.dt == state.dt
...
def test_time_stepper_picard_solver(
    ...
    # Run Picard solver
    final_state = timestepper.picard_solver(state)

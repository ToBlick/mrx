#!/usr/bin/env python3
"""Run one equilibrium per TPU chip, to use all four instead of one.

A ``v5litepod-4`` is four chips and MRX's relaxation occupies one, because the
solve is a chain of dependent matrix-free applies on ~10^4 DoFs with nothing to
shard. A parameter sweep does not have that problem: each equilibrium is
independent, so the chips can each take one.
...
equilibrium per chip and not four copies of one: the states differ, so the
answers must differ, and each must match what it would have been alone.

Previously verified only on four forced host devices
(``XLA_FLAGS=--xla_force_host_platform_device_count=4``). This runs it on real
chips, which is the claim the guide makes.

    python -u pmap_sweep.py --ns 8,16,8 --p 3 --steps 4
"""

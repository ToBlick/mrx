...
def refine(apply_res, solve, b, x0=None, tol=None, project_dual=None, norm=None,
    ...
    working precision's range never limits what the loop can ask for. A
    pass that does not strictly decrease the residual -- or that produces
    a non-finite one -- is discarded and the loop stops on the last
    improving ``x``: in a plain float32 configuration the true residual
    has a floor around 1e-4 on some Hodge / saddle systems (measured
    2026-09-05), and further corrections at ``tol = 1e-6`` raise it, then
    NaN the relaxation (li383 (8,12,12) p=2, step 10). In mixed precision
    each pass
    ...
    def body(carry):
        ...
        x_new = x + s * d.astype(RESIDUAL_DTYPE)

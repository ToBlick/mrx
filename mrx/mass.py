...
class SumfactPlan(NamedTuple):
    ...
    values per component, the per-axis shift plans of the read and of the
    assembly, and the pair structure of the weight. Built once per
    ``(k_row, k_col)`` on the sequence (:func:`mass_plan`,
    :func:`projection_plan`); the weights it is applied with live on the
    geometry (:func:`attach_weights`).

    ``gather_plans`` and ``shift_plans`` are static: each is the per-axis
    slice structure of the DOF map (:func:`_shift_plan`), not a tabulation of
    it, so the apply carries no integer index tensors at all -- neither to
    constant-fold nor to keep resident.
    """
...
def build_sumfact_plan(seq, k_row, k_col, pairs, cols):
    ...
    The element plan -- the basis values and the per-axis shift plans of the
    read and of the assembly -- is built once here and passed to the jitted
    kernel, the basis values as runtime arguments and the plans as static
    ones.
    ...
    # The assembly writes each row component in place, so the output needs no
    # scatter offsets: it is already in component order.

...
class CrossProductProjection:
    ...
    def projection(self, w, u):
        ...
        if self.Λn.k == 1 and self.Λm.k == 2 and self.Λk.k == 1:
            # ∫ Λ[i] (Gw x u) / J dx
            Gw_jk = jnp.einsum('jkl,jk->jl', self.G_jkl, w_h_jk)
            Gw_x_u_jk = jnp.cross(Gw_jk, u_h_jk, axis=1)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, Gw_x_u_jk, self.Q.w/self.J_j)
        elif self.Λn.k == 1 and self.Λm.k == 1 and self.Λk.k == 1:
            # ∫ Λ[i] (w x u) dx
            w_x_u_jk = jnp.cross(w_h_jk, u_h_jk, axis=1)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, w_x_u_jk, self.Q.w)
        elif self.Λn.k == 2 and self.Λm.k == 1 and self.Λk.k == 1:
            # ∫ Λ[i] G(w x u) / J dx
            w_x_u_jk = jnp.cross(w_h_jk, u_h_jk, axis=1)
            G_wxu_jk = jnp.einsum('jkl,jk->jl', self.G_jkl, w_x_u_jk)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, G_wxu_jk, self.Q.w/self.J_j)
        elif self.Λn.k == 2 and self.Λm.k == 2 and self.Λk.k == 1:
            # ∫ Λ[i] (w x G_inv u) dx
            Ginvu_jk = jnp.einsum('jkl,jk->jl', self.G_inv_jkl, u_h_jk)
            w_x_Ginvu_jk = jnp.cross(w_h_jk, Ginvu_jk, axis=1)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, w_x_Ginvu_jk, self.Q.w)
        elif self.Λn.k == 1 and self.Λm.k == 2 and self.Λk.k == 2:
            # ∫ Λ[i] G_inv(w x u) dx
            w_x_u_jk = jnp.cross(w_h_jk, u_h_jk, axis=1)
            G_wxu_jk = jnp.einsum('jkl,jk->jl', self.G_jkl, w_x_u_jk)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, G_wxu_jk, self.Q.w)
        elif self.Λn.k == 2 and self.Λm.k == 1 and self.Λk.k == 2:
            # ∫ Λ[i] (G_inv w x u) dx
            Ginvw_jk = jnp.einsum('jkl,jk->jl', self.G_inv_jkl, w_h_jk)
            Ginvw_x_u_jk = jnp.cross(Ginvw_jk, u_h_jk, axis=1)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, Ginvw_x_u_jk, self.Q.w)
        elif self.Λn.k == 2 and self.Λm.k == 2 and self.Λk.k == 2:
            # ∫ Λ[i] (w x u) / J dx
            w_x_u_jk = jnp.cross(w_h_jk, u_h_jk, axis=1)
            return jnp.einsum("ijk,jk,j->i", self.Λn_ijk, w_x_u_jk, self.Q.w/self.J_j)
        else:

#!/bin/bash
# Sec. 3.4, App. D, Figs. 6, 7: the constrained QA design at (24, 48, 24), p = 3, float64 (paper/ad_recovery.py): the
...
#   bash paper/runs/ad_constrained.sh
. "$(dirname "$0")/common.sh"
export EXTRA_ENV="MRX_DTYPE=float64" CPUS=4 MEM_GB=16
A="--records $RECORDS --stage constrained"

M0=$(sub ad_M0 480 "" paper/ad_recovery.py $A --maxiter 3000 --tag _M0)
for k in 0 1 2 4; do
  sub ad_M10s$k 480 afterok:$M0 paper/ad_recovery.py $A --stop-qa 1.0 --perturb-mm 10 --perturb-seed $k \

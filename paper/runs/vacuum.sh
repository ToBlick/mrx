...
for m in 26 34; do
  sub vs_torus$m 90 "" scripts/analytic_vacuum.py $S --ns $m,$((2 * m)),$((2 * m)) --symmetry none --nfp 1 \
    --out $O/torus_$m
  sub vs_period$m 60 "" scripts/analytic_vacuum.py $S --ns $m,$((2 * m)),$m --symmetry field-period --out $O/period_$m
  sub vs_half$m 60 "" scripts/analytic_vacuum.py $S --ns $m,$((2 * m)),$m --symmetry stellarator --out $O/half_$m
done
...
for p in 1 2 3 4; do
  ...
  sub va_p$p 240 "" scripts/analytic_vacuum.py $A --p $p --ns ${ladder%:} --out $O/p$p
  for n in 20 24 28 32 48 64; do
    m=$((n + p))
    if [ $n -lt 48 ]; then minutes=240 gaps=""; else minutes=600 gaps="--gap-sweeps 0"; fi
    sub va_p${p}_n$n $minutes "" scripts/analytic_vacuum.py $A --p $p --ns $m,$((2 * m)),$m $gaps --out $O/p${p}_n$n
  done
done
...
for p in 2 3 4; do
  ...
  sub vl_p$p 360 "" scripts/qa_vacuum_sweep.py --geometry $LOWRES --p $p --ns ${ladder%:} --out $O/lowres
  for n in 25 29 37 45; do
    sub vl_p${p}_n$n 300 "" scripts/qa_vacuum_sweep.py --geometry $LOWRES --p $p \
      --ns $((n + p)),$((2 * n + 6)),$((n + 3)) --out $O/lowres
  done
  ladder=$(for n in 5 9 13 17 21 29; do m=$((n + p)); printf "%d,%d,%d:" $m $((2 * m)) $m; done)
  VH=$(sub vh_p$p 600 "" scripts/qa_vacuum_sweep.py --geometry $HIGHRES --p $p --ns ${ladder%:} --out $O/highres)
  if [ $p = 3 ]; then VH3=$VH; fi
  for n in 37 45; do
    m=$((n + p))
    sub vh_p${p}_n$n 420 "" scripts/qa_vacuum_sweep.py --geometry $HIGHRES --p $p --ns $m,$((2 * m)),$m \
      --out $O/highres
  done
done
...
RUN=$(sub vq_run 10 afterok:$VH3 paper/vacuum_run.py $R)
sub vq_trace 60 afterok:$RUN scripts/poincare_trace.py --run $R/run --fields ic --lines 160 --periods 400
sub vq_bmag 40 afterok:$VH3 scripts/plot_vacuum_qa.py --geometry $HIGHRES --ns 32,64,32 --p 3 \

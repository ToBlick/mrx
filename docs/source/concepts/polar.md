...
MRX uses C¹. The three polar functions are `1`, `cos θ`, `sin θ` in
barycentric form. `get_xi(nt)` in `mrx/extraction_operators.py` returns their
weights on rings 0 and 1 as an array `xi` of shape `(3, 2, nt)`: the
barycentric coordinates of the unit-circle points `(cos θ_j, sin θ_j)` with
respect to the equilateral control triangle (Toshniwal et al., CMAME 2017;
Holderied, thesis eqs. 5.7-5.9). All weights lie in `[0, 1]` and sum to one
on every ring, so constants are exact. The unit circle is exact whenever
`∂F/∂r` at the axis is a pure `m = ±1` mode; `xi` is a constant of `nt` and
the sequence builds it once (`seq.xi`).

A C² construction (six polar functions from rings 0-2, the quadratic term
sampled at the Greville angles) was measured on the toroid Poisson problem
at `p = 3`: the C² errors equalled the C¹ errors to every printed digit at
10-16% fewer degrees of freedom. It was a k=0-only research variant and is
not in the code; the k >= 1 extractions and the derivative stencils below
encode C¹.
...
dirichlet_out)` pairs. They are stored on the sequence as `seq.g0_grad`
and `seq.g1_curl`, dicts keyed `(dirichlet_in, dirichlet_out)`, and applied
as an indexed gather and segment sum (`.T` is the transpose). The divergence
needs no stencil because `E_3` is a selection. The constructor builds them
when the sequence is polar.

...
with `ΔP_i = P_i - pole`. MRX uses C¹: rings 0 and 1 are replaced by the
three polar functions `1`, `cos θ`, `sin θ` in barycentric form. `get_xi(nt)`
in `mrx/extraction_operators.py` returns their weights on rings 0 and 1 as
an array `xi` of shape `(3, 2, nt)`: the barycentric coordinates of the
unit-circle points `(cos θ_j, sin θ_j)` with respect to the equilateral
control triangle (Toshniwal et al., CMAME 2017; Holderied, thesis eqs.
5.7-5.9). All weights lie in `[0, 1]` and sum to one on every ring, so
constants are exact. The unit circle is exact whenever `∂F/∂r` at the axis
is a pure `m = ±1` mode; `xi` is a constant of `nt` and the sequence builds
it once (`seq.xi`).

## 2. The extraction

`PolarExtractionOperator(Λ, xi, zero_bc)` builds a `MatrixFreeExtraction`
of shape `(n_k, n_k_raw)`; `zero_bc=True` also drops the outer ring for a
Dirichlet condition. Per degree, with `o = 1` under Dirichlet and `0`
otherwise:
...
The bulk rows are copied by `_append_bulk_selector`; the fused rows carry
the `xi` weights (k=0) and their radial and angular differences (k=1, 2),
which is why `E E^T ≠ I` on a polar sequence for k = 0, 1, 2 and
`E_3 E_3^T = I`.
...
and `xi[l, 1, j+1] - xi[l, 1, j]` angularly. `build_grad_stencil_g0` and
`build_curl_stencil_g1` in `mrx/operators.py` build these stencils from the
incidence pattern and `xi` alone, with no mass and no inverse, for all four
`(dirichlet_in, dirichlet_out)` pairs. They are stored on the sequence as
`seq.g0_grad` and `seq.g1_curl`, dicts keyed `(dirichlet_in, dirichlet_out)`,
and applied as an indexed gather and segment sum (`.T` is the transpose).
The divergence needs no stencil because `E_3` is a selection.
...
incidence otherwise. `curl ∘ grad` and `div ∘ curl` are `1e-16` under both
boundary conditions. The relaxation loop advances `B` with this curl, so
`div B` is conserved to round-off along the trajectory instead of to a mass
solver's tolerance.
...
- Interpolation and histopolation on a polar space (`seq.interpolate`) solve
  ...
  ring-0/ring-1 surgery (`_conforming_restriction`); the map fit in
  `mrx/gvec.py` uses this, so the axis of a fitted map is one point per
  `zeta`.
- A `zeta`-dependent axis (a stellarator) would need `xi` per `zeta` plane.
  ...
  default is what runs on every stellarator so far.

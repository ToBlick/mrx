...
| `--reconnect-sustained` | with `--reconnect-eps`: each solve diffuses $B - B_0$ only, $B_0$ the initial field with its seed, i.e. Ohm's law $E = \eta (J - J_0)$ with the initial current as a source: the profile, helicity and pressure are held, the shielding sheets decay into islands that saturate, and a seed stays as an applied perturbation |

...
| `--newton-tol TOL [0.1]`, `--newton-maxiter N [100]` | relative residual tolerance and iteration budget of that MINRES solve; the budget is the parameter (the tolerance is never met): more iterations = a more exact direction = past the resolved floor sooner, 100 holds the floor |
| `--newton-precond {harmonic,laplacian,laplacian2,mass} [harmonic]` | its preconditioner: the harmonic atom (the Laplacian atom with the parallel symbol of the harmonic field divided in, floor `mrx.hessian.HARMONIC_FLOOR = 3`; reaches the floor in a fifth of the Laplacian atom's steps and holds it), the k=1 Laplacian atom, the same squared, or the mass atom |
...
| `--potential-velocity {false,true} [true for the gradient descent]` | the projected force as `curl a + c h` from the k=1 Hodge solve of `curlᵀ load(J × B)` instead of the Leray saddle solve: divergence-free to round-off, the same iterates 20-30% cheaper; Newton and the auxiliary field have their own routes |
| `--cfl C [0.5]` | cap on the line-search step, `C /` the velocity's largest logical CFL number; `inf` disables it |
| `--steps N [100 Newton, 3000 gradient]` | the step budget; a job's time limit is no stop, the checkpoint of every chunk restarts it (`--restart`) |
| `--chunk N [20 Newton, 500 gradient]` | steps per compiled chunk (one `lax.scan`, `mrx.relaxation.chunk_runner`): the per-step trace comes back, the qoi are sampled (helicity, the two pressures and beta, below), a snapshot, the checkpoint and the outputs are written, and the floor, reconnect and wall-time tests run once per chunk; `--steps` is a multiple of it |
| `--floor-tol TOL [1e-8]` | stopping criterion: the last chunk's mean squared normalised force residual `‖F‖²_M / ‖grad(B²/2)‖²` below it |
| `--step-regularisation C [0.1 Newton, 0 gradient]` | the line search minimises the regularised energy $E + \varepsilon \|J\|^2 / 2$ along the increment, $\varepsilon = C / n_r^2$, the direction and the force unchanged: a step whose induction is rough is shortened by about $1 + \varepsilon k^2$, a smooth one untouched (`TimeStepper.step_regularisation`); at the resolved floor the accepted step shrinks to a few per cent of the Newton length |
| `--dt-floor DT [0.1 Newton, 0 gradient]` | stopping criterion: the last chunk's mean accepted step below it (with the regularised line search, the sign that the descent has reached what the mesh resolves) |

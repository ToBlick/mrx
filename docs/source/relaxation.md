...
| `--reconnect-sustained` | with `--reconnect-eps`: each solve diffuses $B - B_\mathrm{ref}$ only, i.e. Ohm's law $E = \eta (J - J_0)$ with $J_0$ the $L^2$ projection of the equilibrium file's current (with the seed) as a source: the profile, helicity and pressure are held, the shielding sheets decay into islands that saturate, and a seed stays as an applied perturbation |

...
| `--method {newton,lbfgs} [newton]` | the direction: Newton on the second variation (the rows below), or the L-BFGS descent |
...
| `--step-regularisation C [0.1 Newton, 0 L-BFGS]` | the line search minimises the regularised energy $E + \varepsilon \|J\|^2 / 2$ along the increment, $\varepsilon = C / n_r^2$, the direction and the force unchanged: a step whose induction is rough is shortened by about $1 + \varepsilon k^2$, a smooth one untouched (`TimeStepper.step_regularisation`); at the resolved floor the accepted step shrinks to a few per cent of the Newton length |
| `--dt-floor DT [0.1 Newton, 0 L-BFGS]` | stopping criterion: the last chunk's mean accepted step below it (with the regularised line search, the sign that the descent has reached what the mesh resolves) |
...
| `checkpoints/state_best.h5` | the field with the lowest per-step squared residual of the run, rebuilt into a state (one force evaluation) and tagged with its step: the run's answer when it went past its floor (the residual is not monotone, and past the resolved floor the ideal descent raises it). `summary.best_step`, `summary.best_resid` in `relax.json` say which step |

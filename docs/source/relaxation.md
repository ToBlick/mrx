...
| `--method {newton,lbfgs} [lbfgs in plain float32, else newton]` | the direction: Newton on the second variation (the rows below), or the L-BFGS descent |
...
| `--force-hodge-passes N [1 in plain float32, else 0]` | outer passes of that k=1 solve, warm-started from the last step's potential; 0 is the solve's own cap. One pass is 37-44% off the float32 step with no loss at the floor (see the MPS page) |

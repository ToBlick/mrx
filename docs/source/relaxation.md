...
| `--newton-tol TOL [0.1]`, `--newton-maxiter N [100]`, `--newton-passes P [3]` | the forcing term (the residual of the Newton system, in double precision and the mass-atom norm, below `TOL` of the right-hand side), the MINRES iterations per pass and the passes at most; the solve is inexact by design, a tighter one gives the same relaxation |

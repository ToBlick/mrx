...
| `--seed m,n,rho0,width[,phase] [""]`, `--seed-eps EPS [0]` | equilibrium files only: a resonant `cos(2π(mθ − s nζ + phase))` term in `A'_ζ` at `rho0` (`EPS` = `|δB^ρ|/|B^ζ|` there) that opens an island of width ~`sqrt(EPS)` at the `|iota| = nfp n/m` surface -- a tearing-stability probe; `phase` (periods, 0 by default) turns the chain, 0.5 swaps its O- and X-points |

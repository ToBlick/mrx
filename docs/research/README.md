...
- `map2disc_2026-09-13.md` — the native map2disc: the blend defect that made every ellipse oracle pass, the degree/Nyquist/boundary resolution laws, why the Newton is warm-started by degree continuation (and why shape continuation is worse than nothing), the interior seed that a crescent needs because its centroid is outside the plasma, and both a QA vacuum solve and an NCSX relaxation on the two maps as physics-level checks.

...
describes what runs today; that is `docs/source/concepts/PRODUCTION.md`. Every file starts
...
- `island_diagnostic_2026-09-18.md` — the island diagnostic (`mrx.poincare.islands`): finds every chain of a state by Newton on the return map (Cary-Hanson fixed points, Greene's residue) and measures its width by tracing the ray through the O-point; the residue is NOT a width (the pendulum conversion overestimates 1.35-1.6; the paper's section widths were confirmed); the (5,1)-seeded chain's phase flip.

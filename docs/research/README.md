...
- `desc_interface.md` — the DESC reader measured three ways: DESC vs VMEC on a grid (R, Z converge; the initial-condition difference contracts 22× through relaxation), a GVEC control on the same wout, a sweep over DESC's own 3-D equilibria, and two DESC-side traps — `VMECIO.load`'s poloidal flip and its half-mesh lambda fit. Backs `docs/source/concepts/external_interfaces.md`.

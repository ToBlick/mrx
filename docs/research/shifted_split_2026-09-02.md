...
> **Read this for:** the split identity for `(M_k + eps L_k)^-1`, its proof, and the measured iteration counts against the saddle MINRES it replaced
> **Do not read for:** the Hodge split of the unshifted Laplacian (`precond_h_scaling_2026-09-02.md`) or the joint `(M + eps S)` atom (OPEN 3.9, not built)
...
The mass atom. On the closed forms of each split system the operator is
exactly `M_j`, where the atom is right. On the coexact part it leaves
`kappa ~ 1 + eps lambda_max(M^{-1} S)`, and the count grows
330 -> 772 -> 1326, about `n_r^2`. The joint separable `(M + eps S)` atom of
OPEN 3.9, built from one consistent 1-D factorisation, would flatten that
and is now a drop-in for each of the two solves.

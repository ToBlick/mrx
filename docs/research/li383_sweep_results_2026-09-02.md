...
**The ladder, `reconnect_ladder_h16_p2_g1` (2026-09-04, job 17461050, `li383_pub.sh reconnect ladder`).** The same rung, `--reconnect-every 2000 --reconnect-eps 0.02 --steps 18000`: eight solves of eps = 7.8e-5 (1.2% of H_0 each) from step 0 at a uniform interval, nine ideal equilibria (eight checkpoints under `reconnect/<k>/` plus the final field), 18000 steps in 10063 s (0.56 s/step). Sections of all ten fields in one call (`poincare/poincare_{ic,reconnect<k>,final}_zeta*.png`, one colour scale); composite `figures/ladder_zeta0.png`; the run's own traces (residual, energy released, CFL number taken as 100-step block means with sd, helicity, J/B, beta, the reconnections dotted, the ideal run dashed) in `figures/ladder_traces.png` with the vector version `figures/pgf/ladder_traces.pgf` (`li383_pub_figures.py ladder_figure`, house style); `figures/reconnect_traces.png` has it next to the three-rung arm. Since 2026-09-04 every `.pgf` and its `-img*.png` companions live in a `pgf/` subfolder of the figure or section directory.

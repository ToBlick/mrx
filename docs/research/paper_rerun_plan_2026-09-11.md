...
| B5 | p sweep p = 1, 3, 4 (fig:p-sweep) | 62 000 / 10 000 / 5000 steps | As B3; the p = 4 descent differs from the paper's arm (steps 0.84 vs 5.6) independently of the refinement stop, see the runtimes note. **p = 5 dropped** (Tobias, 17:40: "it did not do well at all"; no more p = 5 relaxation runs). | 6 h (2.6 + 1.7 + 1.7) | Moderate; p = 1 is the long one (62 000 steps). |

...
| 7 | `one_size_fits_all_map`, `metric_coefficients`, `backtracking_line_search`, `greville_interpolate_stellarator_map` removed; `extend_map_nfp` REWRITTEN (rotates the period map's own output; seam test). Follow-up (branch `analytic-map`, merged): the spline map's R,Z coefficients are built from the series coefficients by L2 projection with closed-form angular symbols, no Greville grid; and the wout radial refit now enforces the full `rho^m` axis behaviour of every mode (QA (24,48,24) residual 3.9e-4 -> 8.4e-5, ‖J‖ 0.37 -> 0.056) | yes |
...
**Suite at 6b1b5f8 (2026-08-28, GPU, MRX_WOUT_GATES=1): 252 passed, 0 failed, 0 skipped.** Merged that day, in order: items 1-5 (5e3b3ee), 7-12 (c25657c), 4 (0345191), 13+18 (be3767a, a95c1e6), 9 unified (51d011b), 20 (089cce1, b3bc494), the Poincaré panels (bacd0ad), the quadrature migration (3247311), the QA vacuum study (28b2e40, 8eaaf46) and the wout axis fixes it triggered (cb65d2c, 3406d38, c532378, 8ab03e8), the analytic map (5094df4..beb1548), the h5-route removal (3210a83..c99236f). Open: 17 (poincare.py split), the dead `iota_spread`/`closed_axes` keys of `load_clebsch`, 22 (docs).

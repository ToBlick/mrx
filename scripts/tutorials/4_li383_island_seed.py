...
from mrx.plotting import section_figure
...
# %%
# Now we take Poincare sections of BOTH initial fields at five planes over half
# a field period (one trace per field, cut at each plane): the island at the
# resonant chain shows in the seeded sections, not the unseeded ones.
for B, tag, title in ((B_unseeded, "unseeded", f"unseeded IC {ns} p={cli.p}"),
                      (B_seeded, f"seeded_eps{cli.seed_eps:g}",
                       f"seeded ({int(m)},{int(n)}) eps={cli.seed_eps:g} {ns} p={cli.p}")):
    res, _ = trace_sections(seq, B, nfp, n_seeds=cli.seeds, n_periods=cli.periods, name=tag)
    for plane in (0.0, 0.125, 0.25, 0.375, 0.5):
        fig, _ = section_figure(seq, res, plane, title=title, nfp=nfp,
...
print("[done] the island at the resonant chain appears in the seeded section only; "
      "the resistive step of Tutorial 5 can reconnect it.")

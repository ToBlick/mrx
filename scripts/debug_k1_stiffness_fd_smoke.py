"""Quick CG-iteration smoke test for the k=1 stiffness tensor preconditioner."""
...
def cg_iters(seq, ops, *, dirichlet: bool, kind: str) -> int:
    key = jax.random.PRNGKey(100 + 10 * int(dirichlet) + SEED_BY_KIND[kind])
...
def main():
    ...
    for rank in (1, 2, 3, 4):
        ...
        print(
            f"rank={rank} dbc   none={none_iter:4d}  "

...
E1f = LazyExtractionOperator(Λ1, ξ, False).M
...
M1f = LazyMassMatrix(Λ1, Q, F, E1f).M

D1 = LazyDerivativeMatrix(Λ1, Λ2, Q, F, E1, E2).M
D1f = LazyDerivativeMatrix(Λ1, Λ2, Q, F, E1f, E2).M
D2 = LazyDerivativeMatrix(Λ2, Λ3, Q, F, E2, E3).M

M21 = LazyProjectionMatrix(Λ1, Λ2, Q, F, E1f, E2).M
...
B_hat = jnp.linalg.solve(M2, P2(B_analytic))
# H_hat = jnp.linalg.solve(M1f, M21.T @ B_hat)
J_hat = jnp.linalg.solve(M2, D1f @ H_hat)
...
J_h = DiscreteFunction(J_hat, Λ2, E2)
def J_phys(x):
    DFx = jax.jacfwd(F)(x)
    G = DFx.T @ DFx
    
    Jr, Jχ, Jz = J_h(x) / jnp.linalg.det(DFx)
    
    return jnp.array([Jr * G[0,0]**0.5,
                      Jχ * G[1,1]**0.5, 
                      Jz * G[2,2]**0.5])
...
# %%
H_h = DiscreteFunction(H_hat, Λ1, E1f)
def H_phys(x):
    DFx = jax.jacfwd(F)(x)
    G = DFx.T @ DFx
    ...
    return jnp.array([Hr / G[0,0]**0.5,
                      Hχ / G[1,1]**0.5, 
                      Hz / G[2,2]**0.5])


plt.plot(_x1, jax.vmap(B_phys)(_x)[:, 2])
plt.plot(_x1, jax.vmap(H_h)(_x)[:, 1])
...
for iter in range(1000):
    ...
    if iter % 50 == 0:
        print(
            f"Iteration {iter}, Force: {force_trace[-1]}, Energy: {Energy_trace[-1]}, Helicity: {Helicity_trace[-1]/Helicity_trace[0] - 1}, Divergence: {divergence_trace[-1]}")
# %%
# %%
...
fig1.savefig('freidberg_energyhelicity_pressureproj.pdf', bbox_inches='tight')
...
fig2.tight_layout()
plt.show()
fig2.savefig('freidberg_invariants_pressureproj.pdf', bbox_inches='tight')
# %%
...
ax.plot(_x1, (jax.vmap(q_h)(_x) - q_h(_x[-1])), label=r'$p / \beta$', color='grey', linestyle="-", lw=LINE_WIDTH)
...
# %%
plt.contourf(_y1, _y2, jax.vmap(q_h)(_x).reshape(nx, nx), cmap='plasma')
plt.colorbar()
# %%
plt.plot(___x1, jax.vmap(p_h)(___x)[:, 0], label=r'$p$')
plt.plot(___x1, jax.vmap(q_h)(___x)[:, 0], label=r'$q$')
plt.legend()
# %%
plt.contourf(_y1, _y2, jax.vmap(u_h)(_x)[:, 2].reshape(nx, nx), cmap='plasma')
plt.colorbar()
# %%

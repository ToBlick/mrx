...
def main(cfg: DictConfig) -> float:
    ...
    print(f"B-field validation error: mean={mean_val_error:.2e}, max={max_val_error:.2e}")
    
    div_B_initial = float(((seq.strong_div @ B_dof_0) @ seq.M3 @ (seq.strong_div @ B_dof_0))**0.5)
    ...
    ts = TimeStepper(
        seq=seq,
        conjugate=cfg.relaxation.conjugate,
    ...
    helicity_change = abs((final_helicity - traces["helicity"][0]) / traces["helicity"][0])
    ...
    if cfg.output.save_final:
        ...
        with h5py.File(output_file, "w") as f:
            ...
            f.create_dataset("force_trace", data=jnp.array(traces["force_norm"]))

...
seq = DeRhamSequence(
        (n, n, n), (p, p, p), 2*p,
        ("clamped", "periodic", "periodic"),
        map=map, polar=True, r_scale=0.66
    )
seq.evaluate_1d()
...
def interpolate_vector_field(x_q, x_axis, y_axis, z_axis, v_values):
    ...
    y_axis_pad = jnp.append(y_axis, y_axis[0] + 1)
    v_grid_pad = jnp.concatenate([v_values, v_values[:, 0:1, :, :]], axis=1)
    
    z_axis_pad = jnp.append(z_axis, z_axis[0] + 1)
    v_grid_pad = jnp.concatenate([v_grid_pad, v_grid_pad[:, :, 0:1, :]], axis=2)
    
    def interpolate_scalar(V_scalar):
        # V_scalar shape is purely (nx, ny, nz)
        interpolator = RegularGridInterpolator(
            points=(x_axis, y_axis_pad, z_axis_pad), 
            values=V_scalar, 
            method='linear' 
        )
        return interpolator(x_q) 

    return jax.vmap(interpolate_scalar, in_axes=-1, out_axes=-1)(v_grid_pad)
...
def interpolate_scalar_field(x_q, x_axis, y_axis, z_axis, v_values):
    ...
    """
    nx, ny, nz = len(x_axis), len(y_axis), len(z_axis)
    v_values = v_values.reshape(nx, ny, nz)  # Reshape to (nx, ny, nz, 3)
    ...
    return interpolator(x_q) 
# %%
_r = pts.reshape(50, 50, 50, 3)[:, 0, 0, 0]
_t = pts.reshape(50, 50, 50, 3)[0, :, 0, 1]
_z = pts.reshape(50, 50, 50, 3)[0, 0, :, 2]
...
p_h_vals = jax.lax.map(p_h, pts[pts[:, 0] < 1.0], batch_size=mrx.MAP_BATCH_SIZE_INNER)
p_vals_valid = p_vals[pts[:, 0] < 1.0][:, None]
print(f"--- p (FEM) ---")
print(f"p={p}, n={n}")
print("Max abs error:", 
...
lambda_h_vals = jax.lax.map(lambda_h, pts[pts[:, 0] < 1.0], batch_size=mrx.MAP_BATCH_SIZE_INNER)
lambda_vals_valid = lambda_vals[pts[:, 0] < 1.0][:, None]
print(f"--- lambda (FEM) ---")
print(f"p = {p}, n={n}")
print("Max abs error:", 
...
phi_h_vals = jax.lax.map(phi_h, pts[pts[:, 0] < 1.0], batch_size=mrx.MAP_BATCH_SIZE_INNER)
phi_vals_valid = phi_vals[pts[:, 0] < 1.0][:, None]
print(f"--- phi (FEM) ---")
print(f"p = {p}, n={n}")
print("Max abs error:", 
...
chi_h_vals = jax.lax.map(chi_h, pts[pts[:, 0] < 1.0], batch_size=mrx.MAP_BATCH_SIZE_INNER)
chi_vals_valid = chi_vals[pts[:, 0] < 1.0][:, None]
print(f"--- chi (FEM) ---")
print(f"p = {p}, n={n}")
print("Max abs error:", 
...
# %%
B_dof = B_guess # seq.apply_leray_projection(B_guess, k=2)[0]
H_dof = seq.apply_leray_projection(H_guess, k=1)[0]
# %%
J_at_pts = jax.lax.map(jacobian_determinant(seq.map), pts, batch_size=mrx.MAP_BATCH_SIZE_INNER)[pts[:, 0] < 1.0]
# %%
...
B_h_vals = jnp.squeeze(jax.lax.map(B_h, pts[pts[:, 0] < 1.0], batch_size=10_000))
B_vals_valid = B_vals[pts[:, 0] < 1.0]
...
# %%
# B_h = jax.jit(Pushforward(DiscreteFunction(B_dof_direct, seq.basis_2, seq.e2_dbc), seq.map, 2))
# B_h_vals = jnp.squeeze(jax.lax.map(B_h, pts[pts[:, 0] < 1.0], batch_size=100_000))
# B_vals_valid = B_vals[pts[:, 0] < 1.0]
# print(f"--- B (FEM - Jax interpolation) ---")
# print(f"p={p}, n={n}")
# print("Max abs error:", jnp.max(jnp.linalg.norm((B_vals_valid - B_h_vals) * J_at_pts[:, None]**0.5, axis=1)) / jnp.mean(jnp.linalg.norm(B_vals_valid * J_at_pts[:, None]**0.5, axis=1)))
# print("Max abs error occurs at:", pts[jnp.argmax(jnp.linalg.norm((B_vals_valid - B_h_vals) * J_at_pts[:, None]**0.5, axis=1))])
# print("Mean abs error:", jnp.mean(jnp.linalg.norm((B_vals_valid - B_h_vals) * J_at_pts[:, None]**0.5, axis=1)) / jnp.mean(jnp.linalg.norm(B_vals_valid * J_at_pts[:, None]**0.5, axis=1)))
...
# %%
# J_hat = seq.apply_weak_curl(B_dof, dirichlet_in=True, dirichlet_out=True)
# J_h_xyz = jax.jit(Pushforward(DiscreteFunction(J_hat, seq.basis_1, seq.e1_dbc), seq.map, 1))
# J_h = jax.jit(DiscreteFunction(J_hat, seq.basis_1, seq.e1_dbc))

J_hat = seq.apply_strong_curl(H_dof, dirichlet_in=False, dirichlet_out=True)
J_h_xyz = jax.jit(Pushforward(DiscreteFunction(J_hat, seq.basis_2, seq.e2_dbc), seq.map, 2))
J_h = jax.jit(DiscreteFunction(J_hat, seq.basis_2, seq.e2_dbc))


__r = jnp.linspace(0.001, 0.999, 1000)
eval_pts = jnp.vstack([__r, jnp.zeros_like(__r), jnp.zeros_like(__r)]).T

J_xyz_at_rad = jax.vmap(J_h_xyz)(eval_pts)
J_at_rad = jax.vmap(J_h)(eval_pts)
Jac_at_rad = jax.vmap(jacobian_determinant(seq.map))(eval_pts)
# %%
plt.plot(__r, J_xyz_at_rad[:, 0], label="J_x")
plt.plot(__r, J_xyz_at_rad[:, 1], label="J_y")
plt.plot(__r, J_xyz_at_rad[:, 2], label="J_z x jacobian(map)")
plt.plot(__r, jnp.linalg.norm(J_xyz_at_rad, axis=1), label="|J|")
plt.xlabel("r")
plt.ylabel("J")
plt.legend()
plt.show()
...
# %%
plt.plot(__r, J_at_rad[:, 0], label="J_r")
plt.plot(__r, J_at_rad[:, 1], label="J_theta")
plt.plot(__r, J_at_rad[:, 2], label="J_zeta")
plt.plot(__r, jnp.linalg.norm(J_xyz_at_rad, axis=1), label="|J|")
plt.xlabel("r")
plt.ylabel("J")
plt.legend()
...
B_at_rad = jax.vmap(B_h)(eval_pts)
# %%
...
# %%
plt.plot(__r, B_at_rad[:, 0], label="B_r")
plt.plot(__r, B_at_rad[:, 1], label="B_theta")
plt.plot(__r, B_at_rad[:, 2], label="B_zeta")
plt.xlabel("r")
plt.ylabel("B")
plt.legend()
plt.show()
...
#         (lambda_h, p_h, phi_h, chi_h), 
#         (lambda_vals, p_vals, phi_vals, chi_vals), 
#         ("lambda", "pressure", "phi", "chi")
#     ):
...
#         mrx.MAP_BATCH_SIZE_INNER = 50_000
#         rhs = seq.P2(B_clebsch)
...
#         B_norm = (B_dof @ seq.apply_m2_sparse(B_dof))**0.5
#         div_B_norm = (seq.apply_strong_div(B_dof) @ seq.apply_d2_sparse(B_dof))**0.5

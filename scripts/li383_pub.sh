...
anchor_sweeps() {
    ...
    # p = 2 gamma = 1 rung of hsweep_p2): gradient descent (the m0 arm; the
    # L-BFGS histories 3, 5 were dropped 2026-09-17), and gamma = 0. 5000 steps, no
    ...
    arm h16_p2_g1_m0 "$GEOM_HI" "$common $G1_16"        300
...
aux() {  # aux [smoke|pairs]
    ...
    if [ "${1:-}" = smoke ]; then
        submit relax aux_smoke "$wt/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 8,16,16 --p 2 --floor-tol 1e-5 --steps 200 --chunk 100 $AUX --out $PUB/aux_smoke" 30
    elif [ "${1:-}" = pairs ]; then
        ...
        for st in B H; do
            flag=""; [ $st = H ] && flag=$AUX
            submit relax h12_p2_f64_$st "$wt/scripts/relax.py" "--method gradient --geometry $GEOM_HI $base --ns 12,24,24 --p 2 $G1_12 --precision float64 $flag --out $PUB/h12_p2_f64_$st" 300
            submit relax h8_p1_$st      "$wt/scripts/relax.py" "--method gradient --geometry $GEOM_HI $base --ns 8,16,16  --p 1 --velocity-smoothing-order 1 $flag --out $PUB/h8_p1_$st" 120
        done
    else
        submit relax aux_h16_p2_g1 "$wt/scripts/relax.py" "--method gradient --geometry $GEOM_HI $common --steps 5000 --out $PUB/aux_h16_p2_g1" 300
    fi
}
...
reconnect() {  # reconnect [smoke|ladder|ladder5k|refine_smoke]
    ...
    if [ "${1:-}" = ladder5k ]; then
        submit relax reconnect_l5_h16_p2_g1 "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 16,32,32 --p 2 $G1_16 $L5 --out $PUB/reconnect_l5_h16_p2_g1" 240
        submit relax reconnect_l5_h32u_p2_g1 "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 32,32,32 --p 2 $G1_32 $L5 --out $PUB/reconnect_l5_h32u_p2_g1" 480
        submit relax reconnect_l5_h32r_p2_g1 "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 32,32,32 --p 2 $G1_32 $R32 $L5 --out $PUB/reconnect_l5_h32r_p2_g1" 540
    elif [ "${1:-}" = refine_smoke ]; then
        # The refined radial grid end to end at n_r = 16: 5 + 5 window cells, 300 steps, one solve.
        submit relax refine_smoke "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 16,16,16 --p 2 $G1_16 --knots-r 0,0.225,0.45,0.49,0.53,0.57,0.61,0.65,0.68,0.728,0.776,0.824,0.872,0.92,1 --floor-tol 0 --steps 300 --chunk 100 --reconnect-every 200 --reconnect-helicity 0.02 --out $PUB/refine_smoke" 40
    elif [ "${1:-}" = ladder ]; then
        submit relax reconnect_ladder_h16_p2_g1 "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 16,32,32 --p 2 $G1_16 --floor-tol 0 --steps 18000 --reconnect-every 2000 --reconnect-helicity 0.0125 --out $PUB/reconnect_ladder_h16_p2_g1" 420
    elif [ "${1:-}" = smoke ]; then
        submit relax reconnect_smoke "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 8,16,16 --p 2 --floor-tol 0 --steps 3000 --chunk 100 --reconnect-every 600 --out $PUB/reconnect_smoke" 40
    else
        submit relax reconnect_h16_p2_g1 "$WT/scripts/relax.py" "--method gradient --geometry $GEOM_HI --ns 16,32,32 --p 2 $G1_16 --floor-tol 0 --steps 8000 --reconnect-every 2000 --out $PUB/reconnect_h16_p2_g1" 400
    fi
}

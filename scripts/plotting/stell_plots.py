...
# %%
    with h5py.File("out/stell/" + name + ".h5", "r") as f:
        B_hat = f["B_final"][:]
        p_hat = f["p_final"][:]
    ...
    # %%
    output_dir = os.path.join("out", "stell", name, "poincare")
    ...
    for plt_nr in range(B_fields.shape[0]):
        ...
        crossings = jax.vmap(lambda x0: get_crossings(
            # m x N x 3
            B_h, x0, F, N=N_cross, phi_targets=[0.33]))(x0s_sorted)

...
for trace in tqdm.tqdm(iter_traces(trace_file)):
    ...
    physical_intersections = jax.vmap(map)(
        logical_intersections.reshape(-1, 3)).reshape(logical_intersections.shape)
...
for i in range(len(plot_data)):
    ...
    figs.append(fig)
    plt.savefig(
        f"{outdir}/poincare_plot_{plot_data[i]['iteration']}.pdf", dpi=dpi)
    plt.show()
# %%

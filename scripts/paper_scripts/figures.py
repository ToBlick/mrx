...
"""The paper's line figures and tables from the run records (login node, matplotlib only).
    ...
    python scripts/paper_scripts/figures.py [--records DIR] [--out DIR] [--only ITEM,...]

--records is the root that scripts/paper_scripts/runs/*.sh write, <records>/<experiment>/<arm> [outputs]. --out gets the paper's
layout: tables/<table>.tex, figs/pgf/<figure>/<stem>.pgf, and figs/<stem>.{pdf,png} to look at [scripts/paper_scripts/build];
...
def write_table(out, name, spec, head, body, pre=()):
    ...
    lines = ["% generated by scripts/paper_scripts/figures.py from the run records; do not edit", *pre,
...
def main():
    ...
    ap.add_argument("--out", default="scripts/paper_scripts/build", help="tables/ and figs/ in the paper's layout [scripts/paper_scripts/build]")

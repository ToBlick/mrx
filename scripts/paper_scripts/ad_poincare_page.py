...
"""Fig. 8 of Sec. 3.4 (qa_poincare_optimized): the end field of the 10 mm draw 0, remeshed (login node, matplotlib).
    ...
    python scripts/paper_scripts/ad_poincare_page.py [--records DIR] [--out DIR]

Reads <records>/shape_optimization/qa_trace_M10s0_remesh.npz (scripts/paper_scripts/runs/ad_trace.sh); writes its field ``final`` as
...
[scripts/paper_scripts/build], as scripts/paper_scripts/poincare_pages.py does for the other pages. The PGF needs pdflatex on PATH.
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(REPO, "scripts", "paper_scripts", "build"),
                    help="poincare/ renders and figs/pgf/ pages [scripts/paper_scripts/build]")
    cli = ap.parse_args()

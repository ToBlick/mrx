...
from mrx.operators import (
    _apply_k1_curlcurl_extracted_tensor_model,
    _apply_k1_curlcurl_regular_forward,
    _apply_k1_curlcurl_regular_tensor_model,
    _assemble_dense_from_apply,
    _assemble_k1_curlcurl_regular_tensor_dense_matrix,
    _assemble_k1_curlcurl_regular_tensor_model,
    apply_stiffness,
    assemble_incidence_operators,
    assemble_mass_operators,
)
from mrx.preconditioners import _tensor_block_indices_k1
...
def _build_sequence(args):
    ...
    operators = None
    operators = assemble_mass_operators(seq, seq.geometry, operators=operators, ks=(2,))
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark the regular-space k=1 curl-curl tensor forward model."
    )
    ...
    parser.add_argument("--rank", type=int, default=1, help="CP rank for the diagonal M2 fits")
    ...
    parser.add_argument("--extracted", action="store_true", help="Benchmark extracted-space k=1 forward apply instead of regular-space")
    ...
    model = _assemble_k1_curlcurl_regular_tensor_model(
        seq,
        rank=args.rank,
    ...
    if args.extracted:
        full_size = seq.n1_dbc if dirichlet else seq.n1
        bulk_indices = None
        if args.bulk_only:
        ...
        def exact_apply(x):
            if bulk_indices is None:
                return apply_stiffness(seq, operators, x, 1, dirichlet=dirichlet)
            full = jnp.zeros((full_size,), dtype=x.dtype)
            full = full.at[bulk_indices].set(x)
            return apply_stiffness(seq, operators, full, 1, dirichlet=dirichlet)[bulk_indices]
        ...
        def exact_apply(x):
            return _apply_k1_curlcurl_regular_forward(operators, x)

        def model_apply(x):
            return _apply_k1_curlcurl_regular_tensor_model(model, x)
    ...
    if args.dense:
        ...
        if args.extracted:
            ...
            approx = _assemble_k1_curlcurl_regular_tensor_dense_matrix(model)
